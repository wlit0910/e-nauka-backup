...
namespace e_nauka
{
    public partial class Formularz_internetowy9 : System.Web.UI.Page
    {
        ...
        protected void Sprawdz_kod_Click(object sender, EventArgs e)
        {
            ...
            if (zadanie1 == "virtual void" && zadanie2 == "public" && zadanie3 == "Ksztalt"
                    && zadanie4 == "override" && zadanie5 == "&")
            {
                Output.Visible = true;
                Output.Text = "Rysuje kolo";
        ...
        {
            Response.Redirect("EL09.aspx");
        }

        protected void Next_Click(object sender, EventArgs e)
        {
            if (Output.Text == "Rysuje kolo") // potem sprawdz z BD status zadania
            {
                string connectionString = ConfigurationManager.ConnectionStrings["MyConnectionString"].ConnectionString;
                int lessonId = 9;                                // ------------------------------------------------------------- zmienić 
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(insertOrUpdateQuery, conn))
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            Response.Redirect("CongratulationsENG.aspx");  // ------------------------------------------------------------- zmienić 
                        }
                        else
                        {
