...
namespace e_nauka
{
    public partial class Formularz_internetowy5 : System.Web.UI.Page
    {
            ...
            if (zadanie1 == "return" && zadanie2 == "return" && zadanie3 == "silnia")
            {
                ...
                if (Param.Text.Length > 0)
                {
                    try
                    {
                        ...
                        Output2.Text = "Factorial: " + wynik.ToString();

                    }
                    catch (Exception ex)
                    {
                        Output2.Visible = true;
                        Output2.Text = "Error! " + ex.Message;
                    }
                }
                else
                {
            ...
            if (Label_Result.ForeColor == Color.ForestGreen) // potem sprawdz z BD status zadania
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(insertOrUpdateQuery, conn))
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            Response.Redirect("EL06.aspx");  // ------------------------------------------------------------- zmienić 
                        }
                        else
                        {
