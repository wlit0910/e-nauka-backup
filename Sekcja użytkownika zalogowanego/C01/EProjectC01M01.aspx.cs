    ...
    public partial class Formularz_internetowy18 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand("SELECT ENNAME FROM MODULE WHERE mID = 1", conn);
                    SqlDataReader reader = cmd.ExecuteReader();

                    while (reader.Read())
                    {
                        Project_module.Text = reader["ENNAME"].ToString();
            ...
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand("SELECT ENNAME FROM LESSONS WHERE mID = 1", conn);
                    SqlDataReader reader = cmd.ExecuteReader();
