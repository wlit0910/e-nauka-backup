    ...
    public partial class Formularz_internetowy110 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand("SELECT NAME FROM MODULE WHERE mID = 2", conn);
                    SqlDataReader reader = cmd.ExecuteReader();

                    while (reader.Read())
                    {
                        Project2_module.Text = reader["NAME"].ToString();                      
                    }
                    reader.Close();
                }
            }
            ...
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand("SELECT NAME FROM LESSONS WHERE mID = 2", conn);
                    SqlDataReader reader = cmd.ExecuteReader();
